locals {
  ...
  cdp_domain                  = local.cdp_domain_global
  ...
  cdp_portal_hosted_zone_id   = local.prod_env ? aws_route53_zone.cdp_prod_domain.0.zone_id : local.nonprod_aws_accounts ? aws_route53_zone.cdp_env_portal_subdomain.0.zone_id : var.cdp_portal_hosted_zone_id
  cdp_portal_hosted_zone_name = local.prod_env ? aws_route53_zone.cdp_prod_domain.0.name : local.nonprod_aws_accounts ? aws_route53_zone.cdp_env_portal_subdomain.0.name : var.env_cdp_portal_subdomain_name
  cdp_portal_full_domain_name = local.prod_env ? aws_route53_zone.cdp_prod_domain.0.name : local.nonprod_aws_accounts ? "portal.${local.cdp_portal_hosted_zone_name}" : "portal.${var.environment}.${local.cdp_portal_hosted_zone_name}"
}
...
resource "aws_route53_zone" "cdp_env_portal_subdomain" {
  count = local.all_aws_accounts ? 1 : 0
  name  = "${var.environment}.ui.${local.cdp_domain}"
}

resource "aws_route53_zone" "cdp_prod_domain" {
  count = local.prod_env ? 1 : 0
  name  = "portal.${local.cdp_domain}"
}

resource "aws_route53_record" "cdp_portal" {
  zone_id = local.cdp_portal_hosted_zone_id
  name    = local.cdp_portal_full_domain_name
  type    = "A"
