...
variable "portal_waf_ip_set_allowlist" {
  description = "List of IPs to be allowed through the WAF"
  type        = list(string)
  default     = null
}

variable "outputs_writer_role_arn" {
  type        = string
  description = "Arn of the role with write access to DynamoDB output metadata storage"
  default     = ""
}