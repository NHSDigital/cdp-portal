...
describe('DeleteUserPage tests', () => {
  it('renders DeleteClient with correct props when valid searchParams are provided', async () => {
    const ui = await DeleteUserPage({
      params: Promise.resolve({ agreement_id: '123' }),
      searchParams: Promise.resolve({ form_id: '456', user_id: '789' }),
    });
    render(ui);
  ...
  it('throws error when form_id is missing', async () => {
    await expect(
      DeleteUserPage({
        params: Promise.resolve({ agreement_id: '123' }),
        searchParams: Promise.resolve({ user_id: '789' }),
      }),
    ).rejects.toThrow('Form ID and user ID must be provided');
  });
  ...
  it('throws error when user_id is missing', async () => {
    await expect(
      DeleteUserPage({
        params: Promise.resolve({ agreement_id: '123' }),
        searchParams: Promise.resolve({ form_id: '789' }),
      }),
    ).rejects.toThrow('Form ID and user ID must be provided');
  });

  it('throws error when form_id is an array', async () => {
    await expect(
      DeleteUserPage({
        params: Promise.resolve({ agreement_id: '123' }),
        searchParams: Promise.resolve({ form_id: ['456'], user_id: '789' }),
  ...
  it('throws error when user_id is an array', async () => {
    await expect(
      DeleteUserPage({
        params: Promise.resolve({ agreement_id: '123' }),
        searchParams: Promise.resolve({ form_id: '456', user_id: ['789'] }),
