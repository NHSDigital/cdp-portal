...
describe('UserDetailsTable tests', () => {
  ...
  it('renders user rows with correct data', () => {
    ...
    expect(rows).toHaveLength(mockUsersToAdd.length);

    rows.forEach((row, index) => {
      const user = mockUsersToAdd[index];
      ...
      const expectedRoleText = {
        ...
        Both: 'Both (Data Analyst and User Manager)',
        UserManager: 'User Manager',
      };

      expect(row.querySelector('[data-cy="role-cell"]')).toHaveTextContent(
        expectedRoleText[user.role],
      );
