...
describe('CheckboxFilters tests', () => {
  ...
  it('does not render options whose show() returns false', () => {
    const whiteLabelKey: CheckboxFiltersProps['whiteLabelKey'] = 'CDP';

    const hiddenOption = CHECKBOX_FILTERS.flatMap((group) =>
      ...
      ({ option }) =>
        ...
        option.show({ whiteLabelKey }) === false,
    );

    if (!hiddenOption) {
      throw new Error(
        'Expected at least one option to be hidden for this whiteLabelKey',
      );
    }

    render(<CheckboxFilters whiteLabelKey={whiteLabelKey} />);

    expect(screen.getByText(hiddenOption.groupName)).toBeInTheDocument();

    expect(
      screen.queryByText(hiddenOption.option.name),
