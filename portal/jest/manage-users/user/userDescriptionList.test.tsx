...
jest.mock('app/shared/common', () => {
  const originalModule = jest.requireActual('app/shared/common'); // grab the real module
  ...
  return {
    ...
    getFormattedTimestamp: jest.fn(
      (ts) => originalModule.getFormattedTimestamp(ts), // call the real function
    ),
  };
});
...
describe('UserDescriptionList tests', () => {
  it('renders the basic details', async () => {
    const result = await UserDescriptionList({ ...props });
  ...
  it('handles when last login is not present', async () => {
    ...
    const missingLastLoginUser: User = {
      ...baseUser,
      application_roles_agreement: ['Analyst'],
  ...
  it('renders change role link only if user agreement is enabled', async () => {
    ...
    const inactive_user_result = await UserDescriptionList({
      ...props,
    ...
    const active_user_result = await UserDescriptionList({ ...props });

    render(active_user_result);
    expect(queryByDataCy?.('change-role-link')).toBeInTheDocument();
    expect(await getByDataCy('change-role-link')).toHaveAttribute(
      'href',
  ...
  const inductionPassedUser: User = {
    ...baseUser,
    application_roles_agreement: ['Analyst'],
    induction: {
      passed: true,
  ...
  it('induction_assessment_passed throws error if whiteLabelKey is not recognised', async () => {
    const fakeKey = 'BUM' as unknown as WhiteLabelKey;
    await expect(
      UserDescriptionList({
        ...props,
        user_details: inductionPassedUser,
    ...
    ).rejects.toThrow('show induction assessment passed entry missing: BUM');
  });
});
