...
describe(
  ...
  () => {
    ...
    it('main portal page', () => {
      ...
      // correct amount of rows and all contain expected data
      cy.get('table.nhsuk-table tbody tr').should(
        'have.length',
        expected_agreements[Cypress.env('BUILD_ENV')].length,
      );

      const agreements = expected_agreements[Cypress.env('BUILD_ENV')];

      agreements.forEach((agreement_id) => {
        cy.contains('tbody td', agreement_id.toUpperCase()).should('exist');
      });
    });
    ...
    it('renders the launch desktop page and returns to previous page', () => {
      ...
      cy.contains('tbody tr', dsa_000000_qa01.toUpperCase()).find('a').click();
...
describe('support admin tests after login', () => {
  it('shows the support admin all expected agreements', () => {
    const allAgreements = expected_agreements[Cypress.env('BUILD_ENV')];

    const filtered_agreements = allAgreements.filter((id) =>
      id.startsWith('dsa-'),
    );

    const irrelevant_agreements = allAgreements.filter(
      (id) => !id.startsWith('dsa-'),
    );
    ...
    cy.get('tbody tr').should(
      ...
      filtered_agreements.length,
    );

    filtered_agreements.forEach((agreement_id) => {
      cy.contains('tbody td', agreement_id.toUpperCase()).should('exist');
    });

    irrelevant_agreements.forEach((agreement_id) => {
      cy.contains('tbody td', agreement_id.toUpperCase()).should('not.exist');
    });
  });
});
