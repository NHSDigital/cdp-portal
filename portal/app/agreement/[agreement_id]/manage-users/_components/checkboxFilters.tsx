...
import { CHECKBOX_FILTERS, Filter, FilterOption } from './consts';
...
function FilterGroup({ id, name, options, whiteLabelKey }: FilterGroupProps) {
      ...
      <div className='nhsuk-form-group'>
        <fieldset className='nhsuk-fieldset'>
          ...
          <div className={styles.checkboxes_small__small}>
            {options.map((option) => {
              const shoudldShow = option.show
                ? option.show({ whiteLabelKey })
                : true;
              if (!shoudldShow) return null;
              return (
                <FilterOptionField
                  filter_group_id={id}
                  key={option.id}
...
export type FilterOptionProps = FilterOption & {
  filter_group_id: string;
};

export function FilterOptionField({
  name,
  id,
  filter_group_id,
}: FilterOptionProps) {
