...
export interface UseSubmissionStatus {
...
export default function useSubmitUsers({
  ...
  async function submitUsers(
    ...
    try {
      ...
      throwAsyncError('Failed submitting users');
    }

    sessionStorage.removeItem(CookieNames.ADD_USER_FORM);
    ...
    document.cookie = `${CookieNames.MANAGE_USERS_SUCCESS_MESSAGE}=${encodeURIComponent(
      success_message,
