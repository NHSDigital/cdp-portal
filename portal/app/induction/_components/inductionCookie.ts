...
export default async function parseInductionCookie(): Promise<{
  ...
  if (!cookies().has(CookieNames.INDUCTION)) return EMPTY_INDUCTION_COOKIE;

  const existing_cookie = cookies().get(CookieNames.INDUCTION)?.value || '{}';
...
export async function setInductionCookie(
  ...
  cookies().set(CookieNames.INDUCTION, JSON.stringify(cookie), {
