...
tf-force-unlock: guard-MODULE guard-BUILD_ENV init ## Create plan for terraform changes
...
.PHONY: npm-run-dev 
npm-run-dev: npm-install ## Runs the portal locally in dev
	cd portal; \
	...
	PORTAL_SERVICE="${PORTAL_SERVICE}" npm run dev
...
lambda-package-noclean: ## Package subset of lambdas without clearing folder (make lambda-package-noclean LAMBDA_LIST=<directory name>
	...
	for lambda in $(LAMBDA_LIST) ; do \
		./scripts/poetry-package.sh $(LAMBDA_DIR_NAME) $$lambda || exit=1 ; \
	done
...
.PHONY: lint
lint: tf-fmt-check python-lint-all prettier-validate-all eslint-validate-all ## Execute all linters 
	@echo "Looks splendid!"
